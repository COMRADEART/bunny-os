...
earlier claim, and therefore owes the earlier records exactly one thing:
that they are still there, byte for byte. A sentence promising that is worth
nothing a year from now; a test that reads the digests is worth the same in
five.

The digests come from ``qualification/companion-agent-providers/preserved-evidence.json``,
written once when this branch was cut. A file that changed, disappeared or
grew fails here — including a change that looks harmless, because "harmless"
is a judgement the digest is not asking for. ``qualification/**`` is ``-text``
in ``.gitattributes``, which is what makes the comparison meaningful across
the two hosts this phase used: without it a Windows checkout would renormalise
line endings and every digest would fail for a reason that has nothing to do
with the evidence.
"""
...
_RECORD = _ROOT / "qualification" / "companion-agent-providers" / "preserved-evidence.json"
...
class PreservedEvidence(unittest.TestCase):
    ...
    def test_the_record_names_the_base_commit_this_branch_was_cut_from(self) -> None:
        self.assertEqual(
            ...
            "65472aa2f43c78e3db16cb4b9f7ff5d244b52e10",
        )
        self.assertEqual(self.record["phase"], "companion-agent-providers")

    def test_every_prior_evidence_file_is_byte_identical(self) -> None:
        preserved = self.record["preservedEvidence"]
        self.assertGreaterEqual(len(preserved), 40, "the record itself looks truncated")
    ...
    def test_no_prior_evidence_directory_gained_a_file(self) -> None:
        ...
        for phase in (
            "companion-linux", "companion-voice",
            "companion-voice-closure", "companion-speech-input",
        ):
