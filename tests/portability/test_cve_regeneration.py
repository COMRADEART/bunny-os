...
class LiveRegenerationTests(unittest.TestCase):
    ...
    def test_the_committed_findings_regenerate_deterministically(self) -> None:
        # The records describe a candidate commit behind HEAD and are
        # regenerated by reading that commit's tree. A shallow clone does not
        # contain it, and the generator correctly refuses rather than measuring
        # whatever is checked out. CI fetches full history so this runs; a
        # shallow working copy reports why it could not.
        declared = json.loads(
        ...
        if not present:
            self.skipTest(
                f"candidate commit {declared[:12]} is absent from this clone; "
                "fetch with depth 0 to run the regeneration check"
            )
