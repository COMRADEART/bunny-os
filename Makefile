...
test-trust:
	$(PYTHON) scripts/task.py test-trust

test-capsules:
	$(PYTHON) scripts/task.py test-capsules

test-app-catalog:
	$(PYTHON) scripts/task.py test-app-catalog

test-capsule-task:
	$(PYTHON) scripts/task.py test-capsule-task

test-capsule-phase:
	$(PYTHON) scripts/task.py test-capsule-phase

# Bunny Model Studio: the training subsystem, which is deliberately not part of
# the image. Runs anywhere; needs no torch, no GPU and no network. The real
# training slice is separate, below, because a test suite that downloads a model
# is a test suite people stop running.
test-model-studio:
	$(PYTHON) scripts/task.py test-model-studio

# The runtime model bridge: validator, registry, adapter loading, authority
# isolation. Runs anywhere; no model, GPU, inference server or network.
test-model-bridge:
	$(PYTHON) scripts/task.py test-model-bridge

# The real chain: a converted adapter applied by a real llama-server, real
# inference, and the real authorization path. Needs an operator-started
# llama-server; see MODEL_BRIDGE_REPORT.md.
model-bridge-slice:
	BUNNY_MODEL_BRIDGE_HEAVY=1 $(PYTHON) -m unittest discover -s tests/model_bridge -t . 	  -p "test_runtime_slice.py" -v

# The evidence run: a real base model, a real optimizer, a real adapter on disk.
# Needs torch, transformers and peft, and either a local base model in
# BUNNY_MODEL_STUDIO_BASE or an explicit BUNNY_MODEL_STUDIO_ALLOW_DOWNLOAD=1.
# Nothing is downloaded without that variable, on any target.
model-studio-slice:
	BUNNY_MODEL_STUDIO_HEAVY=1 $(PYTHON) -m unittest discover -s tests/model_studio -t . \
	  -p "test_training_slice.py" -v

# What this machine could train, and why it cannot train the rest.
model-studio-hardware:
	$(PYTHON) model_studio/bin/bunny-model hardware

# The demonstration run in full: validate, preflight, train, evaluate.
model-studio-demo:
...
# Host-runnable visible Trust / approval demo. Does not boot a guest and
# does not claim a stable-release GO. See demos/08-visible-trust/README.md.
demo-visible-trust:
	$(PYTHON) demos/08-visible-trust/run.py

# Booted-guest Trust / approval journeys (granted then denied via AT-SPI).
# On a host without a composed QCOW2 this records NOT_RUN rather than inventing
# a guest. See demos/09-guest-trust/README.md.
demo-guest-trust:
	$(PYTHON) demos/09-guest-trust/run.py

# Host-runnable product-vision surfaces (Visual Keys, appearance, outcomes,
# memory, first-run, voice story). Not a guest boot and not a release GO.
# See demos/10-product-vision/README.md.
demo-product-vision:
	$(PYTHON) demos/10-product-vision/run.py

vm-guest-trust:
	bash build/scripts/vm-guest-trust.sh
...
# What each of the three renderer modes costs, all three in one process so the
# figures are comparable. Takes an optional path to write the JSON to.
companion-renderer-measure:
	$(PYTHON) scripts/renderer_mode_measure.py
...
# Does the installation medium reach userspace? Nine checkpoints, no disk
# attached, and a screenshot before the first keypress. Run this before any
# installer harness: an ISO that fails here cannot install, and finding that out
# from a fifty-minute timeout with no disk written is how this medium spent its
# first qualification run.
vm-live-boot-checkpoints:
	bash build/scripts/vm-live-boot-checkpoints.sh

# The same question asked of the artifact instead of a machine — under a second,
# and it catches a stale initrd, a missing live payload and a label mismatch.
check-live-media:
	sudo $(PYTHON) build/scripts/check-iso-boot-artifacts.py \
	  --iso "$$(find build/out/live -maxdepth 2 -type f -name '*.iso' -print -quit)"
...
tpm-tamper-drill:
	bash $(TPMQ_SCRIPTS)/tamper_drill.sh $(TPMQ)/evidence
...
DSQ := qualification/display-stack
DSQ_SCRIPTS := $(DSQ)/scripts

display-stack-matrix:
	$(PYTHON) $(DSQ_SCRIPTS)/run_matrix.py

display-stack-evidence-gate:
	$(PYTHON) $(DSQ_SCRIPTS)/import_results.py --dry-run

display-stack-reliability-gate:
	$(PYTHON) $(DSQ_SCRIPTS)/import_results.py

test-display-stack:
	$(PYTHON) -m unittest discover -s tests/display_stack -t .
