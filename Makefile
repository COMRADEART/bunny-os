...
# Re-hashes every attested evidence file against the digest its own record
# carries. Exit 2 on filter damage or an unexplained missing file, 3 when
# only the recorded pre-existing gaps remain, 0 when the trees are whole.
retained-evidence-audit:
	$(PYTHON) scripts/audit_retained_evidence.py \
	  --report qualification/retained-evidence-audit.json

# ---------------------------------------------------------------------------
# dsq-2 — first-login qualification. Its own scenario tree and its own
# authority: dsq-1's records are frozen evidence about the superseded archive,
# and a dsq-1 record imported here would be a record about a different disk.
FLQ := qualification/first-login
FLQ_SCRIPTS := $(FLQ)/scripts

first-login-context:
	$(PYTHON) $(FLQ_SCRIPTS)/create_evidence_context.py \
	  --source-commit "$${BUNNY_INSTALLED_COMMIT:?set BUNNY_INSTALLED_COMMIT (Commit Q)}" \
	  --archive-commit "$${BUNNY_CANDIDATE_COMMIT:?set BUNNY_CANDIDATE_COMMIT (Commit O)}" \
	  --source-archive-digest "$${BUNNY_ARCHIVE_DIGEST:?set BUNNY_ARCHIVE_DIGEST}" \
	  --installation-artifact "$${BUNNY_INSTALLATION_ARTIFACT:?set BUNNY_INSTALLATION_ARTIFACT}" \
	  --output $(FLQ)/evidence-context.json

first-login-matrix:
	$(PYTHON) $(FLQ_SCRIPTS)/run_matrix.py \
	  --disk-source "$${BUNNY_INSTALLATION_ARTIFACT:?set BUNNY_INSTALLATION_ARTIFACT}"

first-login-evidence-gate:
	$(PYTHON) $(FLQ_SCRIPTS)/import_first_login.py --dry-run

first-login-reliability-gate:
	$(PYTHON) $(FLQ_SCRIPTS)/import_first_login.py

test-first-login:
	$(PYTHON) -m unittest discover -s tests/first_login -t .
