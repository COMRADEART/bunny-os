...
# Publish a qualified media tree (build/out/live, or an archived copy of it) as
# a GitHub release carrying the OS build. The tree must still verify against its
# own BUNNY-MANIFEST.json — the publisher refuses anything else. Needs GH_TOKEN
# with contents:write in the environment:
#   GH_TOKEN=... make publish-release TREE=/root/fa-archive/20260822-155710-live
# Extra publisher flags ride through, e.g. PUBLISH_ARGS="--stay-draft".
.PHONY: publish-release
publish-release:
	$(PYTHON) build/scripts/publish_media_release.py --tree "$${TREE:?TREE=<media tree> is required}" $${PUBLISH_ARGS}
