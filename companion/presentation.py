...
EVENT_PHASES: Mapping[str, str] = {
    ...
    "task_state_changed": "",
    # The presentation-surface vocabulary (see companion.events). Drafting and
    # speaking are display facts about the result; losing the connection is an
    # observation about the world. A restoration maps to nothing on its own:
    # the stream resumes into whatever phase it left, and only a later event
    # says otherwise. A degradation likewise moves nothing by itself — the
    # surface loses a rendering it may still be showing, and the reason lands
    # in the status text, exactly as an operation failure does.
    "response_drafting": "presenting_result",
    "speech_started": "speaking",
    "speech_completed": "presenting_result",
...
_STATUS: Mapping[str, str] = {
    ...
    "task_state_changed": "",
    "response_drafting": "Preparing the result.",
    "speech_started": "Speaking.",
    "speech_completed": "Finished speaking.",
    "connection_lost": "Lost contact with the runtime; the task continues on its own.",
    "connection_restored": "",
    "capability_degraded": "Some presentation was reduced; the task continues.",
}
...
def select_presentation(
    ...
    *,
    phase: str = "idle",
    plan_id: str = "",
    plan: CapabilityPresentationPlan | None = None,
) -> PresentationRecommendation:
    ...
    selected = eligible
    if plan is not None:
        # The capability runtime's execution plan is an authorization rather
        # than a hint: whatever this machine could support, the plan caps what
        # this task may draw. A plan that was not granted a start action
        # authorizes no rendering at all.
        if plan.action not in {"start_local", "start_remote"}:
            selected = _degrade(selected, "text-only")
            reasons.append(
                f"capability plan {plan.plan_id} action is {plan.action}; only task text remains"
            )
        else:
            capped = _degrade(selected, plan.presentation_ceiling)
            if capped != selected:
                reasons.append(
                    f"capability plan {plan.plan_id} permits at most {plan.presentation_ceiling}"
                )
            selected = capped
    limited = False
...
# -- Capability-plan authorization ------------------------------------------
#
# The capability runtime answers "what may run here and why"; this bridge
# turns one of its execution plans into the ceiling the renderer must respect.
# Without it the presentation ladder knows what the machine can do but not
# what the *task* was permitted to do — a plan that approved a text-only
# assistant action would still be drawn as a full 3D companion.
...
class CapabilityPresentationPlan:
    """The presentation ceiling one capability plan authorizes."""

    plan_id: str
    service_id: str
    action: str
    implementation_id: str | None
    presentation_ceiling: str
    requires_approval: bool = False
    reasons: tuple[str, ...] = ()

    def __post_init__(self) -> None:
        if self.presentation_ceiling not in PRESENTATION_KINDS:
            raise ValueError(f"unknown presentation ceiling: {self.presentation_ceiling!r}")

    @classmethod
    def from_execution_plan(cls, document: Mapping[str, Any]) -> "CapabilityPresentationPlan":
        """Read this service's decision out of a capability execution plan."""
        ...
        ceiling = _CAPABILITY_IMPLEMENTATIONS.get(str(implementation), "text-only")
        reasons = tuple(
            str(item.get("code"))
