...
Implemented source controls include programmatic button/search labels, keyboard-activatable list rows, visible focus in all Bunny CSS themes, text wrapping, 75–200% text-scale validation, reduced motion, reduced transparency, high contrast, and text-plus-color status. Essential launcher, workspace, approvals, tasks, terminal, lock, screenshot, settings, Search, first-run, and Super+Space command-surface paths have keyboard/mouse alternatives; the companion figure is never required. No gesture is exclusive.
