...
Stable device/user modules deep-link to GNOME Control Center. Bunny owns the Bunny, AI, and Privacy modules (voice listening off until wanted; one AI control — Automatic / Local only / Online enhanced; honest `cloud_context` separate from `remote_dispatch`) plus typed user preferences and Phase 1 broker status. OS updates, previous deployments, rollback, recovery scheduling, and diagnostic export remain separate broker operations with Polkit. Bunny application updates are never merged into the OS update state.
