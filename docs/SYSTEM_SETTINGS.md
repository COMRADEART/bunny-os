...
The Bunny page covers character, voice, personality (presentation only — never a vendor or model name), animation intensity, rendering tier (FULL is the only fully featured tier; BALANCED / LIGHT / MINIMAL are live ceilings on the same character), position and size, interaction, proactivity (off or gentle; gentle may offer and never acts), memory pointers, local/online AI, and privacy pointers. Hide the figure and Search, Settings, and Trust still work. Updates chrome does not claim IMAGE, BOOT, or PASS.
