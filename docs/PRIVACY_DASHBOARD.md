...
Application network chrome is fail-closed: Off or Full internet. Site allowlists are not available yet. `cloud_context=none` is not the same consent as a one-time `remote_dispatch` hop; Trust, Settings Privacy, and Control Center Privacy all say so in the same words.
